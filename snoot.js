...
function copyBillingAddress() {
    ...
    // duplicate addresses - checkbox not checked - erase
    else {
        for (var i = 0; i < billingInputElements.length; i++) {
...
// a function to validate delivery date
...
function validatePayment() {
    ...
    try {
        ...
        if (!cards[0].check && !cards[1].checked && !cards[2].checked && !cards[3].checked) {
            for (var i = 0; i < cards.length; i++) {
                cards[i].style.outline = "5px solid blue";
            }
        } else {
            for (var i = 0; i < cards.length; i++) {
...
// build a function to create our event listeners
